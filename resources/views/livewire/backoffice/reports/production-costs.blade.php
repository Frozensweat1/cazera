<div>
    <div class="space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <div class="panel"><p class="text-sm text-gray-500">Total Cost</p><p class="mt-2 text-2xl font-extrabold"><x-ui.money :amount="$totalProductionCost" /></p></div>
            <div class="panel"><p class="text-sm text-gray-500">Avg Entry</p><p class="mt-2 text-2xl font-extrabold"><x-ui.money :amount="$averageCost" /></p></div>
            <div class="panel"><p class="text-sm text-gray-500">Daily Average</p><p class="mt-2 text-2xl font-extrabold"><x-ui.money :amount="$dailyAverage" /></p></div>
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($productionByModule as $module)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><div><p class="font-semibold">{{ $module->module?->name ?? 'No module' }}</p><p class="text-xs text-gray-500">Entries {{ number_format($module->entries) }}</p></div><strong><x-ui.money :amount="$module->total_amount" /></strong></div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($dailyTrend as $day)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><div><p class="font-semibold">{{ \Illuminate\Support\Carbon::parse($day->production_date)->format('M d, Y') }}</p><p class="text-xs text-gray-500">Entries {{ number_format($day->entries) }}</p></div><strong><x-ui.money :amount="$day->total_amount" /></strong></div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($recentCostEntries as $entry)
                        <div class="rounded-lg border border-slate-200 p-3"><div class="flex justify-between"><strong>{{ $entry->title }}</strong><span><x-ui.money :amount="$entry->amount" /></span></div><p class="text-xs text-gray-500">{{ $entry->branch?->name ?? 'Branch' }} / {{ $entry->production_date?->format('M d, Y') }} / {{ $entry->is_locked ? 'Locked' : 'Open' }}</p></div>
                    @empty
