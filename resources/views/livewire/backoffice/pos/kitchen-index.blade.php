    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="text-gray-500">Track prep queues, move dishes through service, and keep order status current.</p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-2 2xl:grid-cols-3">
            @forelse ($saleItems as $item)
                @php
                    ...
                    $statusClass = $statusClasses[$item->kitchen_status] ?? $statusClasses['pending'];
                    $waitTime = $this->formatWaitTime($item);
                    $imageUrl = $this->menuItemImageUrl($item);
                @endphp

                <article class="panel flex h-full flex-col gap-4 border border-slate-200 p-0">
                    <header class="flex items-start justify-between gap-4 border-b border-slate-100 p-4">
                        <div class="flex min-w-0 gap-3">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="font-mono text-sm font-bold text-slate-900">{{ $item->sale?->sale_number }}</span>
                            </div>
                            <h2 class="mt-2 truncate text-xl font-extrabold text-slate-950">{{ $item->item_name }}</h2>
                            <p class="mt-1 text-sm text-gray-500">
                                {{ $item->sale?->customer?->name ?? 'Walk-in' }} · {{ $item->created_at?->format('h:i A') }}
                            </p>
                            </div>
                        </div>
                        <span class="inline-flex shrink-0 items-center rounded-full border px-3 py-1 text-xs font-bold {{ $statusClass }}">
                    ...
                    <div class="grid grid-cols-3 gap-3 px-4">
                        <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                            <p class="text-xs font-semibold uppercase text-gray-500">Qty</p>
                            <p class="mt-1 text-lg font-extrabold text-slate-950">{{ number_format($item->qty, 0) }}</p>
                        </div>
                        <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                            <p class="text-xs font-semibold uppercase text-gray-500">Wait</p>
                            <p class="mt-1 text-lg font-extrabold text-slate-950">{{ $waitTime }}</p>
                        </div>
                        <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                            <p class="text-xs font-semibold uppercase text-gray-500">Sale</p>
                            <p class="mt-1 truncate text-sm font-bold text-slate-950">{{ ucfirst(str_replace('_', ' ', $item->sale?->status)) }}</p>
                        </div>
                    </div>

                    @if ($item->notes)
                        <div class="mx-4 rounded-lg border border-amber-100 bg-amber-50 p-3 text-sm text-amber-800">
                            {{ $item->notes }}
                        </div>
                    @endif

                    <footer class="mt-auto flex flex-wrap items-center gap-2 border-t border-slate-100 p-4">
            ...
            @empty
                <div class="panel py-12 text-center text-gray-500 xl:col-span-2 2xl:col-span-3">
                    No kitchen orders found.
                </div>
            @endforelse
        </div>

        <div>{{ $saleItems->links() }}</div>
    </div>
</div>
