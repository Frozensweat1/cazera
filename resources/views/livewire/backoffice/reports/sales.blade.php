<div>
    <div class="space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
            ...
            <div class="panel"><p class="text-sm text-gray-500">Gross Sales</p><p class="mt-2 text-2xl font-extrabold"><x-ui.money :amount="$totalRevenue" /></p></div>
            ...
            <div class="panel"><p class="text-sm text-gray-500">Avg Ticket</p><p class="mt-2 text-2xl font-extrabold"><x-ui.money :amount="$averageOrderValue" /></p></div>
            <div class="panel"><p class="text-sm text-gray-500">Debt / Refunds</p><p class="mt-2 text-2xl font-extrabold text-amber-700"><x-ui.money :amount="$debtBalance" /></p><p class="text-xs text-gray-500">Refunds <x-ui.money :amount="$refunds" /></p></div>
        </div>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($moduleBreakdown as $row)
                                <tr><td class="py-3 font-semibold">{{ $row->module?->name ?? 'No module' }}</td><td>{{ number_format($row->orders) }}</td><td><x-ui.money :amount="$row->total_amount" /></td><td class="font-semibold text-emerald-700"><x-ui.money :amount="$row->paid_amount" /></td></tr>
                            @empty
                    ...
                    @forelse ($statusBreakdown as $status)
                        <div class="rounded-lg border border-slate-200 p-3">
                            ...
                            <div class="mt-1 flex justify-between text-sm text-gray-500">
                                <span>Gross <x-ui.money :amount="$status->total_amount" /></span>
                                <span>Paid <x-ui.money :amount="$status->paid_amount" /></span>
                            </div>
                        </div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($topMenuItems as $item)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><div><p class="font-semibold">{{ $item->item_name }}</p><p class="text-xs text-gray-500">Qty {{ number_format($item->total_qty, 2) }}</p></div><span class="font-bold"><x-ui.money :amount="$item->total_revenue" /></span></div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($topCategories as $category)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><div><p class="font-semibold">{{ $category->category_name }}</p><p class="text-xs text-gray-500">Units {{ number_format($category->total_qty, 2) }}</p></div><span class="font-bold"><x-ui.money :amount="$category->total_revenue" /></span></div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($topCustomers as $customer)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><div><p class="font-semibold">{{ $customer->customer?->name ?? 'Guest' }}</p><p class="text-xs text-gray-500">Orders {{ number_format($customer->orders) }}</p></div><span class="font-bold"><x-ui.money :amount="$customer->spent" /></span></div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($topStaff as $staff)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><div><p class="font-semibold">{{ $staff->creator?->name ?? 'Unknown' }}</p><p class="text-xs text-gray-500">Orders {{ number_format($staff->orders) }}</p></div><span class="font-bold"><x-ui.money :amount="$staff->revenue" /></span></div>
                    @empty
