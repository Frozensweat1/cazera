<div>
    <div class="space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
            ...
            <div class="panel"><p class="text-sm text-gray-500">Stock Value</p><p class="mt-2 text-2xl font-extrabold"><x-ui.money :amount="$inventoryValue" /></p></div>
            ...
            <div class="panel"><p class="text-sm text-gray-500">Reorder Exposure</p><p class="mt-2 text-2xl font-extrabold"><x-ui.money :amount="$reorderExposure" /></p></div>
        </div>
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($highValueItems as $item)
                        <div class="rounded-lg border border-slate-200 p-3"><div class="flex justify-between"><strong>{{ $item->name }}</strong><span><x-ui.money :amount="$item->stock_value" /></span></div><p class="text-xs text-gray-500">Qty {{ number_format($item->quantity_on_hand, 2) }} at <x-ui.money :amount="$item->unit_cost" /></p></div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($topSuppliers as $supplier)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><div><p class="font-semibold">{{ $supplier->supplier?->name ?? 'Unknown Supplier' }}</p><p class="text-xs text-gray-500">Items {{ number_format($supplier->item_count) }}</p></div><strong><x-ui.money :amount="$supplier->stock_value" /></strong></div>
                    @empty
            ...
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($valueByCategory as $category)
                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-3"><span class="font-semibold">{{ $category->category_name }}</span><strong><x-ui.money :amount="$category->category_value" /></strong></div>
                    @empty
